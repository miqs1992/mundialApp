    ...
    describe '#update' do
        context "with valid attributes" do
            ...
            it "updates match in the database" do
                ...
                }}.to change{@match.reload.start_time}.to(@time)
            end
            it "redirects to the match page" do
                patch :update, params: {
                    id: @match.id,
        ...
        context "with invalid attributes" do
            it "does not update match" do
                expect{patch :update, params: {
                    id: @match.id,
                    match: { start_time: nil }
                }}.to not_change{@match.reload.start_time}
            end
    ...
    describe '#set_score' do
        context "with valid attributes" do
            it "sets score and changes finished flag" do
                expect{patch :set_score, params: {
                    id: @match.id, match: { score1: 1, score2: 3 }
                }}.to change{@match.reload.score1}.to(1).and change{@match.score2}.to(3).and change{@match.finished}.to(true)
            end
            it "redirects to the match page" do
        ...
        context "with invalid attributes" do
            it "does not set score if score is nil" do
                ...
                }}.to not_change{@match.reload.score1}
            end

            it "does not set score if score is less than zero" do
                expect{patch :set_score, params: {
                    id: @match.id, match: { score1: 1, score2: -3 }
                }}.to not_change{@match.reload.score1}.and not_change{@match.score2}.and not_change{@match.finished}
            end
